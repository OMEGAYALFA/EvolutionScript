...
if ($input->g['new_flink']) {
	include SOURCES . "new_flink.php";
	exit();
}


if (is_numeric($input->g['edit'])) {
	$chk = $db->fetchOne("SELECT COUNT(*) AS NUM FROM featured_link WHERE id=" . $input->gc['edit']);

	if ($chk != 0) {
		include SOURCES . "edit_flink.php";
		exit();
	}
}
...
if ($input->p['action']) {
	if ($settings['demo'] == "yes") {
		header("location: ./?view=manageflink&msg=this_is_demo");
		exit();
	}
	...
	if (is_array($input->p['mid'])) {
		foreach ($input->p['mid'] as $mid) {
			switch ($input->p['a']) {
				case "delete":
					$db->delete("featured_link", "id=" . $mid);
					break;

				case "selfsponsored":
					$setupd = array("user_id" => "0");
					$db->update("featured_link", $setupd, "id=" . $mid);
					break;
...
$paginator = new Pagination("featured_link", $cond);
$paginator->setOrders("id", "DESC");
$paginator->setPage($input->gc['page']);
$paginator->allowedfield($allowed);
$paginator->setNewOrders($input->gc['orderby'], $input->gc['sortby']);
$paginator->setLink("./?view=manageflink&" . $adlink);
$q = $paginator->getQuery();
include SOURCES . "header.php";
echo "<div class=\"site_title\">مدیریت تبلیغ لینکی ویژه</div>
...
<form method=\"post\">
            ...
            <table width=\"100%\" class=\"widget-tbl\">
                <tr>
                    ...
                    <td>
                    <select name=\"مالک : \" onchange=\"selectowner();\" id=\"owner\">
                        <option value=\"\">تمام اعضا</option>
                        <option value=\"single\" ";
...
echo "</td>
        <td align=\"center\">";
echo $paginator->linkorder("expires", "تاریخ انقضا");
echo "</td>
        <td align=\"center\">";
...
while ($r = $db->fetch_array($q)) {
	...
	echo $r['status'];
	echo "</span></td>
        <td align=\"center\"><a href=\"./?view=manageflink&edit=";
	echo $r['id'];
